...
worksheet 模式（docx）可识别并作答以下题型，答案均写入空白处/答题单元格，不改动题面：

- **填空题**（含多空、完形）：就地替换题干内的下划线空。
- **单选题 / 多选题**：把所选选项字母写入答题处或在选项后标注，不覆盖选项文字。
- **判断题**：行内括号写“对/错”，或 True/False 表格中打勾。
- **简答题 / 论述题**：写入题目后的空白段落。
- **匹配题 / 连线题**：把对应字母/序号写入表格或空白处。
- **排序题**：把顺序写入答题空白。
- **计算题**：把结果（必要时含简要步骤）写入空白处。

`examples/` 下提供 `make_sample_quiz.py` 与覆盖全部题型的 `sample-all-types.docx` 供试用。
...
- **worksheet**（仅 docx）：LLM 结构感知模式，处理真实考卷——字母编号小题 (a)(b)(c)、
  选择题、判断表格、答案写进题后空白段落等。
- **auto**：检查文档自动选择。有表格、选择题选项、字母编号小题/分值标注 `[n]`、或 simple
